...
这是一个基于 Go 语言开发的高性能 ASR（语音识别）后端调度服务。它作为前端与底层 Python ASR 推理服务之间的桥梁，提供流式分段批处理、任务并发调度以及数据持久化功能。

## 🌟 核心特性

*   **⚡ 高并发架构**：基于 **异步生产者-消费者模型**，Go 网关负责连接维持，Python Worker 负责计算，利用 Redis 解耦。
*   **📡 全双工通信**：基于 WebSocket 的全双工通信，支持流式音频上传和结果推送。
...
*   **Gateway (Go Backend)**:
    ...
    *   **机制**: 收到音频切片 -> `XADD` 到 Redis Stream -> 立即返回。**不阻塞**等待推理结果。
    *   **性能**: 在负载测试中，单节点维持 500 并发连接时，CPU 占用极其平稳。
*   **Message Broker (Redis)**:
    *   **角色**: 无限容量的缓冲池 (Buffer) 和消息总线。
    *   **机制**: 利用 Pub/Sub 实现结果向特定 WebSocket 会话的动态精确推送。
*   **Worker (Python ASR)**:
    *   **角色**: 纯计算节点 (Stateless)。
    *   **机制**: 从 Redis Stream 消费组获取任务 -> 推理 -> Publish 结果。
...
*   **数据库**: PostgreSQL 14.20 (pgx driver)
*   **配置管理**: 环境变量
