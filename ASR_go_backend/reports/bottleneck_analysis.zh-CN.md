...
*   **500 路并发**: 需要 500 * 0.1 = **50 个 Worker** 才能保证完全不排队。
...
## 6. 韧性极限 (压力测试结果)
我们进行了三项“极限测试” (混沌、风暴、僵尸) 以寻找系统的崩溃临界点。

### 6.1 混沌测试 (Worker Killing)
*   **操作**: 在高负载下随机杀掉 Python Worker 进程。
*   **结果**: Go 后端 **存活** (未崩溃)，但 **体验失败** (在 Worker 宕机期间，短音频成功率跌至 0%)。
*   **洞察**: 系统具备“鲁棒性” (网关不崩)，但缺乏 **高可用性** (计算节点死了任务就挂)。
    *   *修复*: 需要 Kubernetes/Docker Swarm 来实现 Worker 秒级自动重启，以及依赖 Redis Queue 持久化来重试失败任务。

### 6.2 重连风暴 (Thundering Herd)
*   **操作**: 2000 个用户瞬间重连。
*   **结果**: 1000 个成功，1000 个被拒绝 (HTTP 503)。
*   **洞察**: `MAX_CONNECTIONS=2000` 保护机制 **生效中**。
    *   *权衡*: 拒绝用户总比服务器雪崩要好。要支持 5000+ 用户，必须调大 `ulimit` 并实施多网关负载均衡。

### 6.3 僵尸大军 (Slow Loris)
*   **操作**: 1000 个空闲连接长期占用 Socket。
*   **结果**: 系统保持稳定。
*   **洞察**: Go 的 goroutine-per-connection 模型能有效处理空闲连接，内存占用可控。
