...
LONG_AUDIO = "/home/tiger/Projects/ASR_pc_front/recording/long_audio_test.wav"
...
CONCURRENCY_START = 1
CONCURRENCY_STEP = 1  # Increase by 1 each round
MAX_WAIT_SECONDS = 600  # 10 minutes timeout per task
...
def main():
    ...
    try:
        ...
        print("\n" + "="*60)
        print("📋 PHASE 1: Memory Leak Verification")
        print("="*60)
        
        # Test 1: Short audio
        print("\n🎵 Test 1: Short audio (baseline)")
        run_single_test(SHORT_AUDIO, 1, worker_pid, logger)
        time.sleep(5)
        
        # Test 2: Long audio (the critical test)
        print("\n🎵 Test 2: Long audio (memory leak check)")
        result_long = run_single_test(LONG_AUDIO, 1, worker_pid, logger)
        time.sleep(5)
        
        # Check if memory leak is fixed
        if result_long.worker_rss_delta_mb:
            if result_long.worker_rss_delta_mb < 200:  # Less than 200MB = fixed
                print(f"\n✅ MEMORY LEAK FIX VERIFIED!")
                print(f"   Memory delta: {result_long.worker_rss_delta_mb:+.1f} MB (< 200 MB threshold)")
            else:
                print(f"\n⚠️  POTENTIAL MEMORY LEAK STILL EXISTS!")
                print(f"   Memory delta: {result_long.worker_rss_delta_mb:+.1f} MB (> 200 MB threshold)")
        
        # ===== Phase 2: Concurrency Stress Test =====
        print("\n" + "="*60)
        print("📋 PHASE 2: Concurrency Limit Discovery")
        print("="*60)
        print("⚠️  WARNING: System may crash at high concurrency!")
        print("   All results are logged in real-time to prevent data loss.\n")
        
        input("Press Enter to start concurrency stress test (or Ctrl+C to skip)...")
        
        concurrency = CONCURRENCY_START
        while True:
            try:
                results = run_concurrency_batch(SHORT_AUDIO, concurrency, worker_pid, logger)
