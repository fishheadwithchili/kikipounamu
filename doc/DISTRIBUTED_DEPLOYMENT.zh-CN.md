...
本系统支持在局域网内进行分布式部署。你可以利用闲置的笔记本（CPU Worker）来分担主力机（GPU Master）的计算压力，特别是对于**流式分段音频 (Streaming Segmented)** 任务。

> ⚠️ **重要提示**：
> 由于**文件上传 (Batch)** 任务依赖于文件路径，如果 Worker 无法访问 Master 的文件系统（即没有配置 NFS/SMB 共享存储），Worker 将无法处理 Batch 任务。
> **局域网 Worker 极度适合处理大规模流式分段音频任务。**
>
> **免责声明 (Disclaimer)**:
...
3.  开启前端进行音频采集与转录，部分分段请求会被分发到笔记本处理（可以通过笔记本终端日志确认）。
...
**解决方案**: 仅让笔记本处理流式分段任务，或者在两台机器间配置 NFS 共享，确保存储路径一致。

**Q: 笔记本 CPU 处理会不会太慢？**
A: 会比 GPU 慢，但对于流式分段任务，系统会将音频切片并行处理。多一个 Worker 依然能分担并发压力，减少排队时间。
