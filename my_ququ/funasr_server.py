...
# 强制设置 stdout 和 stderr 使用 UTF-8 编码，避免 Windows 下的乱码问题
...
# 创建一个使用 UTF-8 编码的 StreamHandler
...
logging.basicConfig(
    ...
    handlers=[
        ...
        stream_handler,  # 使用明确设置了编码的 handler
    ],
)
...
class FunASRServer:
    def __init__(self, damo_root=None):
        ...
        if not damo_root:
            ...
            project_root = script_dir  # funasr_server.py 在项目根目录
            damo_root = os.path.join(project_root, "models", "damo")
            
            # 确保目录存在
            os.makedirs(damo_root, exist_ok=True)
            
            logger.info(f"使用项目内模型路径: {damo_root}")
        
        self.damo_root = damo_root
        self.asr_model = None
        self.vad_model = None
    ...
    def _load_asr_model(self):
        ...
        try:
            ...
            logger.info(f"ASR模型路径: {model_path}")
            
            with suppress_stdout():
                from funasr import AutoModel

                self.asr_model = AutoModel(
                    model=model_path,  # 使用绝对路径而不是模型ID
                    disable_update=True,
                    device="cpu",
                )
    ...
    def _load_vad_model(self):
        ...
        try:
            ...
            logger.info(f"VAD模型路径: {model_path}")
    ...
    def _load_punc_model(self):
        ...
        try:
            ...
            logger.info(f"标点恢复模型路径: {model_path}")
    ...
    def run(self):
        ...
        # 【改动】使用 self.damo_root，不再设置 MODELSCOPE_CACHE
        cache_path = self.damo_root
        logger.info(f"使用的模型根目录(damo root): {cache_path}")
        
        # 【方案2】完全不设置 MODELSCOPE_CACHE，直接使用绝对路径
        # 不设置环境变量，避免 ModelScope 自动创建额外的目录结构
        logger.info("不设置 MODELSCOPE_CACHE，使用模型绝对路径")
