...
class FunASRManager {
  ...
  getEmbeddedPythonPath() {
    // 获取 Python 路径（统一使用 .venv）
    const isWindows = process.platform === 'win32';
    const pythonExecutable = isWindows ? 'python.exe' : 'python';
    const pythonSubdir = isWindows ? 'Scripts' : 'bin';

    if (process.env.NODE_ENV === "development") {
      // 开发模式：使用项目根目录的 .venv
      return path.join(__dirname, "..", "..", ".venv", pythonSubdir, pythonExecutable);
    } else {
      // 生产模式：使用打包后的 .venv
      return path.join(
        process.resourcesPath,
  ...
  setupIsolatedEnvironment() {
    // 【思路2】不设置 PYTHONHOME 和 PYTHONPATH，让 Python 自己决定
    // 只设置基础环境变量
    ...
    this.logger.info && this.logger.info('设置Python环境（思路2：不设置PYTHONHOME/PYTHONPATH）', {
      pythonExecutable: this.getEmbeddedPythonPath(),
      note: '让Python解释器使用自己的默认配置'
    });
  }

  buildPythonEnvironment() {
    // 【思路2】构建简化的Python环境，不设置PYTHONHOME/PYTHONPATH
    ...
    this.logger.info && this.logger.info('构建Python环境（思路2）', {
      pythonExecutable: this.getEmbeddedPythonPath(),
      note: '不设置PYTHONHOME/PYTHONPATH，让Python自己决定'
    });
  ...
  getModelCachePath() {
    ...
    const projectModelPath = path.join(projectRoot, "models", "damo");

    this.logger.info && this.logger.info('使用项目内模型路径', {
      path: projectModelPath,
      exists: fs.existsSync(projectModelPath)
    });

    // 【改动】如果目录不存在，创建它
    if (!fs.existsSync(projectModelPath)) {
      this.logger.info && this.logger.info('创建模型目录', { path: projectModelPath });
      fs.mkdirSync(projectModelPath, { recursive: true });
    }
  ...
  async _startFunASRServer() {
    try {
      ...
      return new Promise((resolve) => {
        ...
        // 【改动】不再传递 --damo-root 参数，Python 脚本内部会自动使用项目路径
  ...
  async _stopFunASRServer() {
    if (this.serverProcess) {
      try {
        ...
        this.logger.warn && this.logger.warn('Failed to send exit command', error);
      }

      // Wait for process to exit with timeout
      const pid = this.serverProcess.pid;
      const killTimeout = 3000; // 3 seconds timeout
      ...
      if (result === 'timeout') {
        this.logger.info && this.logger.info('FunASR server exit timed out, force killing...', { pid });

        if (process.platform === 'win32') {
          try {
            // Use taskkill to kill process tree on Windows
            require('child_process').execSync(`taskkill /F /T /PID ${pid}`);
            this.logger.info && this.logger.info('Process tree killed successfully via taskkill');
          } catch (killError) {
            this.logger.warn && this.logger.warn('taskkill failed, falling back to process.kill()', killError);
        ...
        this.logger.info && this.logger.info('FunASR server exited gracefully');
      }
  ...
  async stop() {
    this.logger.info && this.logger.info('Stopping FunASR Manager...');
    await this._stopFunASRServer();
    this.logger.info && this.logger.info('FunASR Manager stopped');
  }
  ...
  async findPythonExecutable() {
    ...
    // 如果正在查找中，返回现有的 Promise
    if (this.findPythonPromise) {
      return this.findPythonPromise;
    }

    this.findPythonPromise = (async () => {
      try {
        // 【改动】强制使用 .venv Python，不走任何回退逻辑
        const venvPython = this.getEmbeddedPythonPath();

        this.logger.info && this.logger.info('强制使用.venv Python', {
          path: venvPython,
          exists: fs.existsSync(venvPython)
        });

        // 【改动】如果 .venv Python 不存在，直接报错，不回退
        if (!fs.existsSync(venvPython)) {
          throw new Error(
            `.venv Python 不存在: ${venvPython}\n` +
            `请先运行以下命令创建虚拟环境：\n` +
            `  python -m venv .venv\n` +
        ...
        if (!this.isPythonVersionSupported(version)) {
          throw new Error(
            `.venv Python 版本不支持: ${version.major}.${version.minor}\n` +
            `需要 Python 3.8-3.11`
          );
        }

        this.pythonCmd = venvPython;
        this.logger.info && this.logger.info('使用.venv Python', {
          path: venvPython,
          version: `${version.major}.${version.minor}`
        });
        return venvPython;
      } finally {
        this.findPythonPromise = null;
      }
    })();
  ...
  async checkFunASRInstallation() {
    ...
    if (this.checkInstallationPromise) {
      return this.checkInstallationPromise;
    }

    this.checkInstallationPromise = (async () => {
      try {
        const pythonCmd = await this.findPythonExecutable();
        ...
        const result = await new Promise((resolve) => {
          ...
          checkProcess.stderr.setEncoding('utf8');
        ...
        this.checkInstallationPromise = null;
      }
    })();
  ...
  async transcribeAudioWithProgress(audioBlob, progressCallback = null) {
    const AudioFileManager = require('./audioFileManager');
    const AudioSegmenter = require('./audioSegmenter');

    const audioFileManager = new AudioFileManager(this.logger);
    const audioSegmenter = new AudioSegmenter(this.logger);
    ...
    try {
      ...
      const saveResult = await audioFileManager.saveRecording(audioBlob, timestamp);

      if (!saveResult.success) {
        throw new Error(`保存录音文件失败: ${saveResult.error}`);
      }

      const savedAudioPath = saveResult.filePath;
      const audioFileRelativePath = saveResult.relativePath;

      this.logger.info && this.logger.info('录音文件已保存:', savedAudioPath);
      ...
      const segmentResult = await audioSegmenter.segmentAudio(savedAudioPath);

      if (!segmentResult.success) {
        throw new Error(`音频分段失败: ${segmentResult.error}`);
      }

      const segments = segmentResult.segments;
      const segmentCount = segments.length;
      const needsSegmentation = segmentResult.needsSegmentation;

      this.logger.info && this.logger.info(`音频分段完成，共 ${segmentCount} 段${needsSegmentation ? '（需要分段处理）' : '（无需分段）'}`);
      ...
      for (let i = 0; i < segmentCount; i++) {
        ...
        if (progressCallback) {
          progressCallback({
            stage: 'transcribing',
            message: `正在转录第 ${i + 1}/${segmentCount} 段...`,
            current: i + 1,
            total: segmentCount
          });
        }

        this.logger.info && this.logger.info(`开始转录第 ${i + 1}/${segmentCount} 段:`, segment.segmentPath);

        const result = await this._sendServerCommand({
          action: 'transcribe',
          audio_path: segment.segmentPath,
          options: {}
        });
        ...
        if (result.success) {
          ...
          this.logger.error && this.logger.error(`第 ${i + 1} 段转录失败:`, result.error);
          transcriptionResults.push({
            success: false,
            text: '',
            raw_text: '',
      ...
      if (needsSegmentation) {
        await audioSegmenter.cleanupSegments(segments);
      }
      ...
      const mergedResult = audioSegmenter.mergeTranscriptions(transcriptionResults);

      if (!mergedResult.success) {
        throw new Error(`合并转录结果失败: ${mergedResult.error}`);
      }

      this.logger.info && this.logger.info('转录完成，文本长度:', mergedResult.text.length);

      return {
        success: true,
        text: mergedResult.text,
        raw_text: mergedResult.raw_text,
        confidence: mergedResult.confidence,
        language: mergedResult.language,
        duration: mergedResult.duration,
        audio_file_path: audioFileRelativePath,
        segment_count: segmentCount,
        was_segmented: needsSegmentation
      };

    } catch (error) {
      this.logger.error && this.logger.error('带进度的转录失败:', error);
      throw error;
    }
  }
