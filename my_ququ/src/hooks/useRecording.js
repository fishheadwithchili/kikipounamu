...
export const useRecording = () => {
  ...
  // 监听转录进度
  useEffect(() => {
    if (window.electronAPI && window.electronAPI.onTranscribeProgress) {
  ...
  const processAudio = useCallback(async (audioBlob) => {
    ...
    try {
      ...
      if (window.electronAPI) {
        ...
        // 使用新的带进度的转录方法
        const transcriptionResult = await window.electronAPI.transcribeAudioWithProgress(uint8Array);
        ...
        if (transcriptionResult.success) {
          ...
          const transcriptionData = {
            ...
            audio_file_path: transcriptionResult.audio_file_path || null
          };
          ...
          setTimeout(async () => {
            try {
              ...
              if (window.electronAPI) {
                ...
                // 显示录音保存提示
                if (transcriptionResult.audio_file_path) {
                  if (window.electronAPI && window.electronAPI.log) {
                    window.electronAPI.log('info', '录音已保存:', transcriptionResult.audio_file_path);
                  }
                }
          ...
          // 转换失败但录音已保存
          if (transcriptionResult.audio_file_path) {
            setError('转录失败，但录音已保存，可在历史记录中查看');
          }
